...
const demoCaptions: Record<string, string> = {
  "robots-txt": "Same file. One line is the entire difference between an AI crawler reading you, or not.",
  "schema-markup": "Same listing, same question asked. One of these tells the AI exactly what it's looking at.",
};
...
export async function generateMetadata({
  ...
  if (!lesson) return {};
  return pageMetadata({
    title: `${lesson.title} — The Free GEO Guide`,
...
const analogyIcons = [clockIcon, dieIcon];

export default async function GeoLessonPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  return (
    <main className="flex-1 px-6 pb-24 pt-40">
      <div className="mx-auto max-w-2xl">
        ...
        {/* Why it matters */}
        {lesson.whyCustomer && lesson.whyAI && (
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              ...
              <Reveal delay={80}>
                <div className="h-full rounded-2xl border border-accent/20 bg-accent/5 p-5">
                  <p className="text-xs font-medium text-accent-text">The AI&rsquo;s side</p>
                  {lesson.whyAIBadge && (
                    <p className="mt-3 inline-block rounded-full bg-ink px-2.5 py-1 text-[11px] font-medium text-ink-foreground">
                      {lesson.whyAIBadge}
                    </p>
                  )}
                  <p className="mt-3 text-sm text-foreground/70">{lesson.whyAI}</p>
                </div>
              </Reveal>
            </div>
          </div>
          ...
          {lesson.diagnoseMethods ? (
            ...
            <>
              ...
              {lesson.diagnoseLink && (
                ...
                >
                  {lesson.diagnoseLink.href.startsWith("/") ? "Open" : "Open"} {lesson.diagnoseLink.label} →
                </a>
              )}
            </>
          )}
            ...
            {next ? (
              ...
              <>
                ...
                <p className="mx-auto mt-2 max-w-sm text-sm text-foreground/65">
                  More of the guide is on its way. Rather we just handled all of this
